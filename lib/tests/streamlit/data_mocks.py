...
SHARED_TEST_CASES = [
    ...
    (
        {"name": [], "type": []},
        TestCaseMetadata(0, 2, DataFormat.COLUMN_VALUE_MAPPING),
    ),
    ...
    ({"st.number_input"}, TestCaseMetadata(1, 1, DataFormat.SET_OF_VALUES)),
    (
        ("st.text_area", "st.number_input", "st.text_input"),
...
LIST_TYPES_DF = pd.DataFrame(
    {
        ...
        + [None],
        "datetime_list": [[random_date() for _ in range(2)] for _ in range(4)]
        + [None],
    }
)
